...
// 33v get the stub and check if that file exists and get habari to load it
// 1 of 2
...
// debug::out();
				// single post
				$post = $posts; //$posts instanceof Post ? $posts : reset( $posts );
				// Stack::add( 'body_class', Post::type_name( $post->content_type ) ); //'-' . $post->id
			}
			else {
				$post = reset( $posts );
		...
		elseif( file_exists($test_file)  ) {
		// 33v 2 of 2
			// file exists, now we create a blank post to activate habari into loading the file
			$post = new Post();
			$post->slug = $stub;
			$post->status =  Post::status('published');
			$post->content_type = 2; //FIXME hard coded, type number can potentially be changed on other installs
			$post->title = '';
			$post->content = '';
		...
		elseif ( ( $posts === false ) ||
			...
			if ( $this->template_exists( '404' ) ) {
				...
				$this->display( 'header' );
				echo '<h2>';
				_e( "404 The content requested doesnt exist" );
				echo '</h2>';
				header( 'HTTP/1.1 404 Not Found', true, 404 );
				$this->display( 'footer' );
				die;
